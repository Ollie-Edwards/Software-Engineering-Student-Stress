...
def test_score_returns_int_or_rounded(task_factory):
    ...
    task = task_factory(
        importance=5, length=60, due_at=datetime.now() + timedelta(hours=24)
    )
...
def test_urgent_task_scores_higher_than_non_urgent(task_factory):
    ...
    task_relaxed = task_factory(
        importance=5, due_at=datetime.now() + timedelta(hours=100)
    )
...
def test_short_task_scores_slightly_higher_than_long_same_conditions(task_factory):
    ...
    task_short = task_factory(
        importance=5, length=15, due_at=datetime.now() + timedelta(hours=24)
    )
    task_long = task_factory(
        importance=5, length=480, due_at=datetime.now() + timedelta(hours=24)
    )
...
def test_high_importance_high_urgency_boost_capped_at_99_5(task_factory):
    ...
    task = task_factory(
        importance=10, length=30, due_at=datetime.now() + timedelta(hours=2)
    )
    score = scoreTask(task)
...
def test_minimum_importance_task_far_deadline_low_score(task_factory):
    ...
    task = task_factory(
        importance=1, length=30, due_at=datetime.now() + timedelta(hours=500)
    )
    score = scoreTask(task)
    assert score < 20
...
def test_custom_emergency_buffer(task_factory):
    ...
    task_no_emergency = task_factory(
        length=60, due_at=datetime.now() + timedelta(hours=1.5)
    )
    task_emergency = task_factory(
        length=60, due_at=datetime.now() + timedelta(hours=1.5)
    )
...
def overdue_task_scores_100(task_factory):
    ...
    # values of "due_in"
    task = task_factory(
        length=60, due_at=datetime.now(timezone.utc) - timedelta(hours=10)
    )
