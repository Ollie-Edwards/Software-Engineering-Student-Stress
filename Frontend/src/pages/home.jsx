...
export default function Home({ isAdding, setIsAdding }) {
  ...
  const Taskcard = (task) => {
    ...
    const priorityLabel =
      task.priority >= 80 ? "High" : task.priority >= 40 ? "Medium" : "Low";
    ...
    const toggleSubtask = async (subtask) => {
      ...
      try {
        const response = await fetch(
          `http://localhost:8000/subtask/${subtask.id}/${endpoint}`,
          {
            method: "POST",
          },
        );
    ...
    return (
      <div
        ...
        className="cursor-pointer relative flex border rounded-2xl shadow-sm overflow-hidden bg-white"
      >
        <div
        ...
        <div className="p-5 flex-1 flex flex-col gap-3">
          ...
          <div className="flex justify-between items-start">
            <div>
              <span
                style={{ backgroundColor: badgeBg, color: badgeText }}
                className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase mb-1 inline-block`}
              >
            ...
            <div className="flex items-center gap-1">
              ...
              >
                {task.completed ? (
                  <svg
                    ...
                    strokeWidth={1.5}
                    stroke="currentColor"
                    className="w-6 h-6 text-slate-400 hover:text-black"
                  >
              ...
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  className={`h-5 w-5 transform transition-transform ${showSubtasks ? "rotate-180" : ""}`}
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                >
          ...
          <div className="grid grid-cols-2 gap-y-2 gap-x-4 pt-4 border-t mt-auto text-xs text-slate-500">
            ...
            <p>
              <strong>Due:</strong> {timeUntil(task.due_at)}
            </p>
          </div>
        </div>
      </div>
  ...
  const handleCreateTask = async (e) => {
    ...
    const newTaskData = {
      ...
      due_at: formData.get("due_at")
        ? new Date(formData.get("due_at")).toISOString()
        : null,
      reminder_enabled: formData.get("reminder") === "on",
      completed: false,
    };
